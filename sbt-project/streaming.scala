...
// TODO optimize this ugly code!!!
  def main(args: Array[String]) {
    ...
    val sparkConf = new SparkConf()
      .setAppName("streamingRecommendingSystem168")
    ...
    val topKMostSimilarMovies =
      movie2movieList.map(i => (i._1, i._2.take(100).map(l => l._1))).collectAsMap
    ...
    val dataDStream =
      ssc.socketTextStream("thumm01", 4321).filter(l => l.split(" ").length == 3).map { l =>
        ...
        rdd
          .map {
            case (userId, movieId, rate) =>
              ...
              Logger.getLogger(this.getClass()).error(dataString)
              if (dataString.length > 0) {
                // println(dataString)
                recentRatings = dataString
                  .split("\\|")
              ...
              val candidateMovies = bTopKMostSimilarMovies.value
              ...
              for (cmovieId <- candidateMovies; (rmovieId, rate) <- recentRatings) {
                ...
                val sim = bMovie2movieSimilarity.value
              ...
              val updatedRecommends = allSimilars.toArray
                  ...
                  case (movieId, simArray) =>
                    (
                      ...
                      simArray.map(_._2).sum / simArray.length + math
                        .log(increaseCounter.getOrElse[Int](movieId, 1)) / math.log(2) - math
              ...
              Logger.getLogger(this.getClass()).error(userId.toString)
              Logger.getLogger(this.getClass()).error(content)
